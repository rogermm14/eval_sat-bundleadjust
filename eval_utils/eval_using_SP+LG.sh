...
if [[ "$KEYPT2SUBPX" == "true" ]]; then
  FEATURE_TYPE_VALUE="superpoint_keypt2subpx"
  RUN_NAME="eval_using_SP+LG_keypt2subpx/eval_${PAIR_SELECTION_MODE}_similarK5_clean_tracks_image_outlier_topk${IMAGE_OUTLIER_TOP_K}_thr3_SIFT_Cauchy"
else
  FEATURE_TYPE_VALUE="superpoint"
  RUN_NAME="eval_using_SP+LG/eval_${PAIR_SELECTION_MODE}_similarK5_clean_tracks_image_outlier_topk${IMAGE_OUTLIER_TOP_K}_thr3_SIFT_Cauchy"
fi
...
echo "Running SuperPoint+LightGlue evaluation"
